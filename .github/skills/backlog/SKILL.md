...
name: backlog
description: >
  Use this skill any time a task involves the Backlog.md CLI — creating, editing,
  viewing, searching, or managing project tasks. Trigger when the user mentions
  "backlog", "task", "create task", "edit task", "acceptance criteria", "definition
  of done", "task status", "assign task", "implementation plan", "final summary",
  or references task IDs (e.g. "task 42"). Also trigger when the user asks to
  manage project work items, check off criteria, or update task metadata.
---

# Backlog.md CLI — Comprehensive Project Management
...
| Edit task | `backlog task edit 42 -s "In Progress" -a @myself` |
| Search | `backlog search "topic" --plain` |
| Check AC | `backlog task edit 42 --check-ac 1` |
| Mark done | `backlog task edit 42 -s Done` |
